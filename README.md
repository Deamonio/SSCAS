...
**SSCAS (Subway Station Congestion Analysis System)**는 AI 객체 인식 기술을 활용하여 지하철역의 혼잡도를 **실시간으로 분석하고 시각화**하는 프로토타입 시스템입니다. 
...
이 프로젝트는 **간이 역 모형**을 제작하여 테스트되었습니다: 
- 축소 모형 지하철역 환경
- ESP32-CAM 카메라 배치
- 실시간 승객 이동 시뮬레이션
...
- **메인 페이지 (`/`)**: 실시간 CCTV 스트림, 현재 승객 수, 혼잡도 히트맵
- **경로 페이지 (`/path`)**: 추가 페이지 (향후 확장 가능)
- **행사 페이지 (`/concert`)**: 추가 페이지 (향후 확장 가능)
